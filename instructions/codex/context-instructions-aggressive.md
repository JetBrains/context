...
When code discovery *is* needed and you don't yet know the right file, subsystem, implementation, or related test, your first discovery action MUST be `jbcontext search` — never open with `grep`/`rg`/`find` or git history. This mandate governs *how* you start code discovery, not whether every task needs a search.
