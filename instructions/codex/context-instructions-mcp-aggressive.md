...
### MANDATORY: jbcontext-first, then iterate

When code discovery *is* needed and you don't yet know the right file, subsystem, implementation, or related test, your first discovery action MUST be `code_search` — never open with `grep`/`rg`/`find` or git history. This mandate governs *how* you start code discovery, not whether every task needs a search.

`code_search` is your **primary tool for locating code**, used **iteratively** — not a one-shot bootstrap.
...
- If that does not pin down the exact place to change, **search again right away**: narrow to the best directory with `pathFilter`, or rephrase the query. Keep refining until the target is located.
