...
Use short, discriminative terms from the request: package names, module names, framework names, SDK names, team names, service names, or explicit repository names. The query matches repository names and what a repository does, so when the name is unknown describe its purpose or stack in a few words instead.
...
```bash
jbcontext search --git-remote-url "<repository>" --json-output --limit 10 "<semantic dependency search query>"
```
